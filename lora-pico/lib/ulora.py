...
class LoRa(object):
    def __init__(self, spi_channel, interrupt, this_address, cs_pin, reset_pin=None, freq=915.0, tx_power=14,
                 modem_config=ModemConfig.Bw125Cr45Sf128, receive_all=True, acks=False, crypto=None):
        """
...
#        gpio_interrupt = Pin(self._interrupt, Pin.IN, Pin.PULL_DOWN)
    ...
    def _handle_interrupt(self, channel):
        ...
        if self._mode == MODE_RXCONTINUOUS and (irq_flags & RX_DONE):
            ...
            # Clear RX IRQ flags ASAP so DIO0 can re-arm even if callback is slow
            ...
            if packet_len >= 4:
                ...
                if not header_flags & FLAGS_ACK:
                    try:
                        # User callback must stay non-blocking (see LoRa main.py P0)
